...
namespace LessonChallengeSolutions.TimeComplexity
{
    public class FrogJmp
    {
        ...
        {
            int startCoordinate = X;
            int endCoordinate = Y;
            int distance = D;
            
            int totalNumberOfJump = 0;

            if (startCoordinate >= endCoordinate)
            {
                return totalNumberOfJump;
            }

            try
            {
                int jumpStart = (endCoordinate - startCoordinate) / distance;

                startCoordinate = startCoordinate + (distance * jumpStart);
                totalNumberOfJump = jumpStart;
            }
            catch
            {
            ...
            {
                startCoordinate = startCoordinate + distance;
                totalNumberOfJump++;
            }
