...
namespace LessonChallengeSolutions.CountingElements
{
    public class FrogRiverOne
    {
        ...
        public int Solution(int X, int[] A)
        {
            ...
            var counterList = new HashSet<int>();
            var aimList = new HashSet<int>();
            ...
            for (int i = 0; i < A.Length; i++)
            {
                ...
                if(counterList.Count < aimList.Count)
                {
                    continue;
                }
                ...
                try
                {
                    if (counterList.Distinct().Intersect(aimList).ToList().Count >= aimList.Count)
                    {
                        return i;
                    }
                }
